...
<form method="post" action="{{ route('profile') }} " enctype="multipart/form-data">
  ...
  <label for="name" class="profilename">ユーザー名</label>
	...
	<div><input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" /></div>
  ...
  <div><input type="text" name="bio" id="bio" value="{{ old('bio', $user->bio) }}" /></div>


  <label class="profilename">アイコン画像</label>
	 <div><input type="file" name="icon_image">
    @error('icon_image')
